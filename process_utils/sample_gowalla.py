...
# File path
folder = "./src/IR-Benchmark-Dataset/data_iid/gowalla"
...
# ----------------------------------------------------------------------------- #
# 2. Randomly sample 1/5 of users
# ----------------------------------------------------------------------------- #

# Get all user IDs (skip header)
all_users = [int(row[1]) for row in user_list[1:]]  # user_procid

# Randomly sample 1/5 of users
sample_size = max(1, len(all_users) // 5)
...
# ----------------------------------------------------------------------------- #
# 5. Update user list and item list
# ----------------------------------------------------------------------------- #

# Build old mapping (from original user_list and item_list)
old_user_map_dict = {int(row[0]): int(row[1]) for row in user_list[1:]}  # {user_orgid: user_procid}
old_item_map_dict = {int(row[0]): int(row[1]) for row in item_list[1:]}  # {item_orgid: item_procid}
...
write_tsv([(u, i) for u, i in remapped_train], f"{folder}/train.tsv")
write_tsv([(u, i) for u, i in remapped_test], f"{folder}/test.tsv")
